...
@testset "SolverResults" begin
    ...
    @testset "Different samplers" begin
        ...
        sr_halton = SolverResults(basis_fns, 1, ops, :quasirandom, HaltonSample(), 1.0, -0.5, 1, [-0.5], [ones(1, 1)], [-0.5])
        sr_sobol = SolverResults(basis_fns, 1, ops, :quasirandom, SobolSample(), 1.0, -0.5, 1, [-0.5], [ones(1, 1)], [-0.5])
