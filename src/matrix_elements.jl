...
flatten1(x) = x isa AbstractVector{<:AbstractVector} ? vec(x[1]) : vec(x)
...
function compute_matrix_element(bra::Rank0Gaussian, ket::Rank0Gaussian, op::Coulomb, w::AbstractVector)
    A, B = bra.A, ket.A
    R = inv(A + B)
    β = 1 / (dot(w, R * w))
    M0 = (π^size(R, 1) / det(A + B))^(3 / 2)
...
function compute_matrix_element(bra::Rank1Gaussian, ket::Rank1Gaussian, op::Coulomb, w::AbstractVector)
    A, B = bra.A, ket.A
...
function compute_matrix_element(bra::Rank1Gaussian, ket::Rank1Gaussian, op::Kinetic, K::AbstractMatrix)
    A, B = bra.A, ket.A
    a, b = flatten1(bra.a), flatten1(ket.a)
    R = inv(A + B)
    M0 = (π^size(R, 1) / det(A + B))^(3 / 2)
...
function compute_matrix_element(bra::Rank2Gaussian, ket::Rank2Gaussian, op::Kinetic, K::AbstractMatrix)
    A, B = bra.A, ket.A
...
function compute_matrix_element(bra::Rank2Gaussian, ket::Rank2Gaussian, op::Coulomb, w::AbstractVector)
    A, B = bra.A, ket.A
    a, b = flatten1(bra.a), flatten1(bra.b)
    c, d = flatten1(ket.a), flatten1(ket.b)
    R = inv(A + B)
    ...
    term2 = -2 * sqrt(β / π) * β / 3 * 0.25 * (q2_1 + q2_2 + q2_3 + q2_4 + q2_5 + q2_6) * M0
    term3 = 2 * sqrt(β / π) * β^2 / 10 * 0.5 * (q4_1 + q4_2 + q4_3) * M0
